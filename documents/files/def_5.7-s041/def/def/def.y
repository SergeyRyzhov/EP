...
pin_option: '+' K_SPECIAL
        ...
        | '+' K_LAYER { dumb_mode = 1; } T_STRING
          ...
          pin_layer_spacing_opt pt pt
          {
            if (defrReader::get()->getPinCbk() || defrReader::get()->getPinExtCbk()) {
              if (hasPort)
                 defrReader::get()->getPin().defiPin::addPortLayerPts($7.x, $7.y, $8.x, $8.y);
              else
                 defrReader::get()->getPin().defiPin::addLayerPts($7.x, $7.y, $8.x, $8.y);
            }
          }
        ...
        | '+' K_VIA { dumb_mode = 1; } T_STRING '(' NUMBER NUMBER ')'   // 5.7
          {
            if (defVersionNum < 5.7) {
              ...
              if (defrReader::get()->getPinCbk() || defrReader::get()->getPinExtCbk()) {
                if (hasPort)
                   defrReader::get()->getPin().defiPin::addPortVia($4, (int)$6,
                                               (int)$7);
                else
                   defrReader::get()->getPin().defiPin::addVia($4, (int)$6,
                                               (int)$7);
              }
            }
          }
...
layer_stmt: '+' K_RECT {dumb_mode = 1;no_num = 1; } T_STRING pt pt 
            {
              if (defrReader::get()->getViaCbk())
                defrReader::get()->getVia().defiVia::addLayer($4, $5.x, $5.y, $6.x, $6.y);
            }
        | '+' K_POLYGON { dumb_mode = 1; } T_STRING
            {
              if (defVersionNum < 5.6) {
            ...
            firstPt nextPt nextPt otherPts
            {
              if (defVersionNum >= 5.6) {  // only add if 5.6 or beyond
                if (defrReader::get()->getViaCbk())
                  defrReader::get()->getVia().defiVia::addPolygon($4, defrReader::get()->getGeomPtr());
              }
            }
...
comp_routehalo: '+' K_ROUTEHALO NUMBER { dumb_mode = 2; no_num = 2; } T_STRING T_STRING
      {
        if (defVersionNum < 5.7) {
           ...
           if (defrReader::get()->getComponentCbk())
             defrReader::get()->getComponent().defiComponent::setRouteHalo((int)$3, $5, $6);
        }
      }
          ...
          firstPt nextPt nextPt otherPts
          {
            if (defVersionNum >= 5.6) {  // only add if 5.6 or beyond
              if (defrReader::get()->getSNetCbk()) {
                ...
                defrReader::get()->getNet().defiNet::addPolygon($4, defrReader::get()->getGeomPtr(), &needSNPCbk);
        ...
        | '+' K_RECT { dumb_mode = 1; } T_STRING pt pt
          {
            ...
            if (defrReader::get()->getSNetCbk()) {
              ...
              // This will improve performance
              // This construct is only in specialnet
              defrReader::get()->getNet().defiNet::addRect($4, $5.x, $5.y, $6.x, $6.y, &needSNPCbk);
              if (needSNPCbk && defrReader::get()->getSNetPartialPathCbk()) {
                 CALLBACK(defrReader::get()->getSNetPartialPathCbk(), defrSNetPartialPathCbkType,
                          &defrReader::get()->getNet());
                 defrReader::get()->getNet().defiNet::clearRectPolyNPath();
