...
int snetf(defrCallbackType_e c, defiNet* net, defiUserData ud) {
           ...
           while ((path = (int)p->defiPath::next()) != DEFIPATH_DONE) {
              switch (path) {
                case DEFIPATH_LAYER:
                     ...
                     sNLayerName = p->defiPath::getLayer();
                     break;
                case DEFIPATH_VIA:
                     if (!ignoreVN)
                     ...
                     break;
                case DEFIPATH_VIAROTATION:
                     fprintf(fout, " %d", p->defiPath::getViaRotation());
                     nline = 1;
                     break;
  ...
  if (net->numPolygons()) {  // 5.6
    struct defiPoints points;
    for (i = 0; i < net->numPolygons(); i++) {
      fprintf(fout, "\nSNET %s POLYGON %s", net->name(), net->polygonName(i));
  ...
  if (net->numRectangles()) {  // 5.6
     for (i = 0; i < net->numRectangles(); i++)
       fprintf(fout, "\nSNET %s RECT %s %d %d %d %d", net->name(),
               net->rectName(i), net->xl(i), net->yl(i), net->xh(i),
               net->yh(i));
  }
...
int cls(defrCallbackType_e c, void* cl, defiUserData ud) {
  ...
  case defrPinCbkType :
         ...
         if (pin->hasLayer()) {
             ...
             for (i = 0; i < pin->numVias(); i++) {
                 fprintf(fout, "+ VIA %s %d %d ", pin->viaName(i),
         ...
         for (j = 0; j < pin->numAntennaModel(); j++) {
             ...
             if (aModel->hasAPinMaxAreaCar()) {
                fprintf(fout, "PIN %s + NET %s %s ", tmpPinName,
                        pin->netName(), aModel->antennaOxide());
                for (i = 0; i <
                     aModel->defiPinAntennaModel::numAPinMaxAreaCar(); i++) {
  ...
  case defrTrackCbkType :
         ...
         for (i = 0; i < track->numLayers(); i++) {
            fprintf(fout, "TRACKS %s %g DO %g STEP %g LAYER %s\n",
                 track->macro(), checkDouble(track->x()),
                 checkDouble(track->xNum()), checkDouble(track->xStep()),
                 track->layer(i));
         }
         break;
  case defrGcellGridCbkType :
         ...
         fprintf(fout, "GCELLGRID %s %d DO %d STEP %g\n",
                 ...
                 gcg->defiGcellGrid::xNum(),
                 checkDouble(gcg->defiGcellGrid::xStep()));
         break;
  case defrViaCbkType :
         via = (defiVia*)cl;
  ...
  case defrSlotCbkType :
         slot = (defiSlot*)cl;
         for (i = 0; i < slot->defiSlot::numRectangles(); i++) {
            fprintf(fout, "SLOT LAYER %s", slot->defiSlot::layerName());
            fprintf(fout, " RECT %d %d %d %d\n",
                    slot->defiSlot::xl(i), slot->defiSlot::yl(i),
                    slot->defiSlot::xh(i), slot->defiSlot::yh(i));
         }
         for (i = 0; i < slot->defiSlot::numPolygons(); i++) {
  ...
  case defrFillCbkType :
         fill = (defiFill*)cl;
         for (i = 0; i < fill->defiFill::numRectangles(); i++) {
         ...
         for (i = 0; i < fill->defiFill::numPolygons(); i++) {
