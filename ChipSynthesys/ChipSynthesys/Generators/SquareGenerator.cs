    ...
    public class SquareGenerator : IGenerator
    {
        public void All_Division(int sizeX, int sizeY, int components, int MaxSizeX, int MaxSizeY, int percent, List<int> parts)
        {
            ...
            if (components <= percent_square)
            {
                ...
                Division(sizeX, sizeY, parts, 0);
                do
                {
                    int i = Control(percent_square, parts, MaxSizeX, MaxSizeY);
                    if (i < 0) break;
                    Division(parts[i], parts[i + 1], parts, i);
                } while (true);
            }
            else
            {
        ...
        public void NextDesignWithPlacement(int components, int nets, int maxNetSize, int percent, int maxSizeX, int maxSizeY,
            int width, int height, out Design design, out PlacementDetail placement)
        {
            int sizeX = 5;
            int sizeY = 3;
            var rnd = new Random();
            List<int> parts = new List<int>();
            All_Division(sizeX, sizeY, components, maxSizeX, maxSizeY, percent, parts);
