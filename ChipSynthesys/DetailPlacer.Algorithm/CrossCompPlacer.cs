...
namespace DetailPlacer.Algorithm
{
    public class CrossCompPlacer : IDetailPlacer
    {
        ...
        public void AddCells(
            ...
            int[] YCellCoord, 
            Component current, 
            List<List<Component>> CompInCell)
        {
            ...
            for (int i = 0; i < sizey; i++)
            {
                ...
                {
                    int index0 = indexbest + j + i * Width;
                    if (index0 < ValueCells.Length)
                    {
                        if (ValueCells[index0] != -1 && xCoord + j <= Width + design.field.beginx
                            && xCoord >= design.field.beginx && yCoord + i <= Height + design.field.beginy
                            && yCoord >= design.field.beginy)
                        {
                            ValueCells[index0]++;
                            CompInCell[index0].Add(current);
                        }
                    }
                }
            }
            ...
            List<List<Component>> CompInCell)
        {
            ...
            int sizex = current.sizex;
            int sizey = current.sizey;
            int indexfirst = ((int)xCoord - design.field.beginx)
                             + ((int)yCoord - design.field.beginy) * design.field.cellsx;

            for (int i = 0; i < sizey; i++)
            {
                for (int j = 0; j < sizex; j++)
                {
                    if (xCoord + j <= Width + design.field.beginx && xCoord >= design.field.beginx
                        && yCoord + i <= Height + design.field.beginy && yCoord >= design.field.beginy)
                    {
                        int index0 = indexfirst + j + i * Width;
                        if (index0 < ValueCells.Length && ValueCells[index0] > 0) //bug
                        {
                            ValueCells[index0]--;
                            CompInCell[index0].Remove(current);
                        }
                    }
                }
            }
        ...
        public double CloselyCell(PlacementGlobal myApproximate, Component current, int XcurrCoord, int YcurrCoord)
        {
            ...
            double area = (maxX - minX) + (maxY - minY); //bug wat +
            return area;
        }

        public int[] CreateCompValueCells(int size)
        {
            ...
            List<List<Component>> CompInCell)
        {
            ...
            foreach (Component comp in design.components)
            {
                if (result.placed[comp] == false)
                {
                    ...
                    if (OutOfField(design, (int)xCoord, (int)yCoord)) continue;

                    index = ((int)xCoord - design.field.beginx)
                            + ((int)yCoord - design.field.beginy) * design.field.cellsx;
                    ...
                    for (int j = 0; j < sizey; j++)
                    {
                        for (int k = 0; k < sizex; k++)
                        {
                            if (xCoord + k <= Width + design.field.beginx 
                                ...
                                && yCoord >= design.field.beginy)
                            {
                                int index0 = index + j * Width + k;
                                if (index0 < ValueCell.Length)
                                {
                                    ValueCell[index0]++; //лишнее
                                    CompInCell[index0].Add(comp);
                                }
                                else
                                {
//                                    Console.WriteLine("Error");
//
//                                    Console.WriteLine(design.field.beginx);
//                                    Console.WriteLine(design.field.beginy);
//                                    Console.WriteLine(design.field.cellsx);
//                                    Console.WriteLine(design.field.cellsy);
//                                    Console.WriteLine(index0);
//                                    Console.WriteLine(xCoord);
//                                    Console.WriteLine(yCoord);
//                                    Console.WriteLine(sizex);
//                                    Console.WriteLine(sizey);
//                                    Console.WriteLine(comp.sizex);
//                                    Console.WriteLine(comp.sizey);
                                }
                            }
                        }
                    }
            ...
            int end)
        {
            ...
            double percentBest = PercentCross(
                XCellCoord[indexOfBest], 
                YCellCoord[indexOfBest], 
                Current, 
                design, 
                myApproximate, 
                result);
            double areaBest = CloselyCell(myApproximate, Current, XCellCoord[indexOfBest], YCellCoord[indexOfBest]);
            double area2Best = NearNet(
                Current, 
                myApproximate, 
                design, 
                XCellCoord[indexOfBest], 
            ...
            {
                var ind = array[i];
                if (CanNotBePlaced(ValueCell[ind], XCellCoord[ind], YCellCoord[ind], Current, design)) continue;

                double percentCurrent = PercentCross(
                    XCellCoord[ind], 
                    YCellCoord[ind], 
                    Current, 
                    design, 
        ...
        {
            return xCoord < design.field.beginx || yCoord < design.field.beginy
                   || xCoord >= Width - design.field.beginx || yCoord >= Height - design.field.beginy;
        }

        public double PercentCross(
            int XCellCoord, 
            int YCellCoord, 
            Component Current, 
            Design design, 
            ...
            PlacementDetail result)
        {
            ...
            foreach (Component next in design.components)
            {
                if (next != Current && result.placed[next] == true)
                {
                    ...
                    {
                        percent = ((maxX - minX) * (maxY - minY) * 100)
                                  / ((maxYcurrent - minYcurrent) * (maxXcurrent - minXcurrent));
        ...
        public virtual void Place(Design design, PlacementGlobal approximate, out PlacementDetail result)
        {
            ...
            foreach (Component comp in design.components)
            {
                if (result.placed[comp] == false)
                {
                    ...
                    int indCurrent = ((int)approximate.x[comp] - design.field.beginx)
                                     + ((int)approximate.y[comp] - design.field.beginy) * design.field.cellsx;
        ...
        private bool CanNotBePlaced(int cell, int x, int y, Component component, Design design)
        {
            ...
            return cell < 0 || x + component.sizex > field.cellsx + field.beginx
