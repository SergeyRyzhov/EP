...
namespace DetailPlacer.Algorithm.PositionSearcher.Impl
{
    public abstract class PositionSearcherBase : IPositionSearcher
    {
            ...
            out int[] x, out int[] y, out bool hasPosition)
        {
            ...
            var success = DetourPositions(n, m, mask, addIfTheLimitIsNotExceeded);
            if (!success)
            {
        ...
        protected abstract bool DetourPositions(int n, int m, int[,] mask,
            Func<int, int, bool> addIfTheLimitIsNotExceeded);
